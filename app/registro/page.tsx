...
export default function AuthForm({ onLoginSuccess }: AuthFormProps) {
    ...
    const handleVerify = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            if (response.ok && data.token && data.usuario) { // O data.user según lo que retornes
                setMessage({
                    type: 'success',
                ...
                if (onLoginSuccess) {
                    onLoginSuccess(data.usuario, data.token); // O data.user
                } else {
        ...
        try {
            ...
            const response = await fetch('https://-back.vercel.app/api/auth/reset-password', {
                ...
                body: JSON.stringify({
                    ...
                    code: resetData.code,
                    newPassword: resetData.newPassword // ✅ Asegúrate de enviar esto
                }),
            });
