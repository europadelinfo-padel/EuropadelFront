...
// ============================================
// 📁 VendedorActivoPage.tsx - CON ENVÍO DE PRODUCTOS
// ============================================
"use client";
...
const VendedorActivoPage: React.FC = () => {
  ...
  const cambiarRol = async (id: string, nuevoRol: 'usuario' | 'vendedor'): Promise<void> => {
    try {
      ...
      // Mostrar mensaje si se enviaron productos
      if (nuevoRol === 'vendedor' && data.message) {
        alert(data.message);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Error desconocido';
  ...
  const enviarProductosManual = async (id: string, nombre: string): Promise<void> => {
    if (!window.confirm(`¿Enviar todos los productos disponibles a ${nombre}?\n\nEsto creará una copia de cada producto admin en su catálogo personal.`)) {
      return;
    }
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/vendedoractivo/${id}/enviar-productos`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const data = await response.json();
      const cantidadProductos = data.data?.productosCreados || 0;
      
      alert(
        `✅ Productos enviados exitosamente a ${nombre}\n\n` +
        `📦 ${cantidadProductos} productos creados\n` +
        `📧 Email de notificación enviado`
      );
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Error desconocido';
      console.error('Error:', errorMessage);
      alert('Error al enviar productos al vendedor');
    } finally {
      setProcesando(prev => ({ ...prev, [id]: false }));
    }
  };
              ...
              {vendedores.map((vendedor) => (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      {vendedor.rol === 'vendedor' ? <UserX size={16} /> : <UserCheck size={16} />}
                      {vendedor.rol === 'vendedor' ? 'Convertir a Usuario' : 'Convertir a Vendedor'}
                    </button>
